...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    int adjustY = (MASK_X % 2) ? 1 : 0;
    int xBound = MASK_X /2;
    int yBound = MASK_Y /2;
    __shared__ int shared_mask[MASK_N][MASK_X][MASK_Y];
    shared_mask[(x+y)%MASK_N][(x+y)/MASK_N%MASK_X][(x+y)/MASK_N/MASK_X%MASK_Y] = mask[(x+y)%MASK_N][(x+y)/MASK_N%MASK_X][(x+y)/MASK_N/MASK_X%MASK_Y];
    __syncthreads();
...
int main(int argc, char** argv) {
    ...
    std::cout << (width + dimBlock.x - 1) / dimBlock.x << " " << (height + dimBlock.y - 1) / dimBlock.y << std::endl;
