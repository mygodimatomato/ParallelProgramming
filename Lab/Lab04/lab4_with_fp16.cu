...
__device__ char mask[Z][Y][X] = { { { -1, -4, -6, -4, -1 },
                                        { -2, -8, -12, -8, -2 },
                                        { 0, 0, 0, 0, 0 },
                                        { 2, 8, 12, 8, 2 },
                                        { 1, 4, 6, 4, 1 } },
...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    for (int i = 0; i < Z; ++i) {
        ...
        val[i][2] = CUDART_ZERO_FP16;
        val[i][1] = CUDART_ZERO_FP16;
        val[i][0] = CUDART_ZERO_FP16;
    ...
    __half totalR = CUDART_ZERO_FP16;
    __half totalG = CUDART_ZERO_FP16;
    __half totalB = CUDART_ZERO_FP16;
    for (int i = 0; i < Z; ++i) {
        totalR += val[i][2] * val[i][2];
        totalG += val[i][1] * val[i][1];
        totalB += val[i][0] * val[i][0];
    }
    totalR = __hdiv(hsqrt(totalR), SCALE);
    totalG = __hdiv(hsqrt(totalG), SCALE);
    totalB = __hdiv(hsqrt(totalB), SCALE);
    __half half255 = __float2half(255.);
    const unsigned char cR = (totalR > half255) ? 255 : __half2float(totalR);
    const unsigned char cG = (totalG > half255) ? 255 : __half2float(totalG);
    const unsigned char cB = (totalB > half255) ? 255 : __half2float(totalB);
...
int main(int argc, char **argv) {
    ...
    // cudaMemcpyToSymbol(const_mask, mask, Z * Y * X * sizeof(char));
    // decide to use how many blocks and threads
    const int num_threads = 256;
    const int num_blocks = width*height / num_threads + 1;
