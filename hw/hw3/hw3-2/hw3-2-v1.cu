...
__global__ void phase2(int* d_dist, int r){
  ...
  for(int k = 0; k < BLOCK_SIZE; k++){
    ...
    col[i+0][j] = min(col[i+0][j], pivot[i+0][k] + col[k][j]);
    col[i+1][j] = min(col[i+1][j], pivot[i+1][k] + col[k][j]);
    col[i+2][j] = min(col[i+2][j], pivot[i+2][k] + col[k][j]);
    col[i+3][j] = min(col[i+3][j], pivot[i+3][k] + col[k][j]);
  }
  __syncthreads();
...
void block_FW(int* d_dist) {
  ...
  for (int r = 0; r < round; r++) {
    ...
    phase2<<<round, num_threads, 2 * BLOCK_SIZE * BLOCK_SIZE * sizeof(int)>>>(d_dist, r);
